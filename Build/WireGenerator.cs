...
namespace AdventOfCode2019
{
    class WireGenerator
    {
        private WireCross Wire1 = new WireCross();
        private WireCross Wire2 = new WireCross();
        public struct XYLocation
        {
            public int x;
            public int y;
            public int manhatnum;
        }
        public List<XYLocation> CloseColl = new List<XYLocation>();
  
        public int ManhattanNumber
        {
            get { ManhattanDistance(); return CloseColl[0].manhatnum; }
        }
        private void ManhattanDistance()
        {
            XYLocation ClosestCollison = new XYLocation { manhatnum = Math.Abs(Wire1.XCoordCol(0)) + Math.Abs(Wire1.YCoordCol(0)), x = Wire1.XCoordCol(0), y = Wire1.YCoordCol(0) };

            for (int i = 0; i< Wire1.NumOfColNodes();i++)
            {
                int temp = Math.Abs(Wire1.XCoordCol(i)) + Math.Abs(Wire1.YCoordCol(i));
                if(temp < ClosestCollison.manhatnum)
                {
                    ClosestCollison.manhatnum = temp;
                    ClosestCollison.x = Wire1.XCoordCol(i);
                    ClosestCollison.y = Wire1.YCoordCol(i);
                }
            }
            CloseColl.Add(ClosestCollison);
        }

        public int CollisonLength()
        {
            int wireLengthMin = 0;
            int wireTemp1 = 0;
            bool tempFoundCol = false;
            
            for (int a = 0; a < Wire1.NumOfColNodes(); a++)
            {
                ...
                while(!tempFoundCol)
                {
                    tempFoundCol = Wire1.LengthOfWire(Wire1.RtnCoordinate(i), Wire1.RtnCoordinate(i + 1), Wire1.XCoordCol(a), Wire1.YCoordCol(a));
                    if(tempFoundCol)
                    {
                        wireTemp1 = Wire1.LenOfWire;
                        Wire1.LenOfWire = 0;
                    }
                    i++;
                }
